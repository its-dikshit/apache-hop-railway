...
if "%~1"=="" (
    echo Usage: Outlets_cleanup.bat [source_name]
    exit /b 1
)
...
:: ======= Run update_outlets_mdm.py =======
if exist "%UPLOAD_SCRIPTS_DIR%\update_outlets_mdm.py" (
    echo Running update_outlets_mdm.py with source_name: "%SOURCE_NAME%"
    python "%UPLOAD_SCRIPTS_DIR%\update_outlets_mdm.py" "%SOURCE_NAME%"
) else (
    echo Error: update_outlets_mdm.py not found in Upload Scripts
    exit /b 1
)
